...
if os.path.isfile('config.csv') and os.path.isfile('quiz_data.csv') and os.path.isfile('question_data.csv'):
...
with open('config.csv' , newline='', encoding='utf-8-sig') as csvconfig:
	...
	config_data = next(config_reader)

headers = {'Authorization' : 'Bearer '+config_data["User_Access_token"]}
url = 'https://reykjavik.instructure.com//api/v1/courses/'+config_data["Course_ID"]+'/quizzes'

#Fetch quiz data
with open('quiz_data.csv' , newline='', encoding='utf-8-sig') as csvquiz:
...
with open('question_data.csv' , newline='', encoding='utf-8-sig') as csvfile:

	#Column reader used to determine maximum number of viable answers.	
	column_reader, row_reader = itertools.tee(csv.reader(csvfile, delimiter=';'))
...
print('Done. Go to https://reykjavik.instructure.com/courses/'+config_data["Course_ID"]+'/question_banks where you can assign your questions to a question bank.')
